...
export class AuthService {
  ...
  async signup(signupDto: SignupDto) {
    ...
    // Send verification email in background — don't block the signup response
  ...
  async verifyEmail(token: string) {
    ...
    this.emailService
      ...
      .catch((error) => this.logger.error(`Failed to send welcome email to ${user.email}: ${error.message}`));
  ...
  async forgotPassword(emailOrUsername: string, captchaToken: string) {
    ...
    // Send password reset email in background — don't block the response
    const language = user.preferred_language || 'en';
    this.emailService.sendPasswordResetEmail(user.email, password_reset_token, user.username, language)
      .catch(error => {
  ...
  async resend2FACode(userId: string) {
    ...
    // 2FA code must be awaited — user needs the code to proceed
    const language = user.preferred_language || 'en';
    try {
      await this.emailService.send2FACode(user.email, twoFactorCode, user.username, language);
    } catch (error) {
      this.logger.error(`Failed to send 2FA code to ${user.email}: ${error.message}`);
      throw new BadRequestException('Failed to send verification code. Please try again.');
    }
  ...
  async resendVerificationEmail(email: string) {
    ...
    this.emailService.sendVerificationEmail(
      ...
      this.logger.error(`Failed to resend verification email to ${user.email}: ${error.message}`);
    });
