  ...
  async handleStripeSubscriptionCancelled(stripeSubscriptionId: string): Promise<void> {
    const subscription = await this.db.queryOne(
      `SELECT s.id, s.user_id, s.current_period_end,
              u.email, u.username, u.preferred_language,
              p.name as pack_name, p.is_free
       FROM subscriptions s
       JOIN users u ON s.user_id = u.id
       JOIN packs p ON s.pack_id = p.id
    ...
    if (subscription.is_free) {
      this.logger.log(`Free subscription ${subscription.id} cancelled (no kick or email needed)`);
      return;
    }
    ...
    if (otherActiveSubscriptions.length === 0) {
      ...
      try {
        ...
        this.logger.log(`Subscription ended email sent to ${subscription.email} (${language})`);
      } catch (emailError) {
        this.logger.error(`Failed to send subscription ended email to ${subscription.email}:`, emailError);
      }
    } else {
      this.logger.log(
        `User ${subscription.user_id} still has ${otherActiveSubscriptions.length} active paid subscription(s), not kicking from Telegram or sending expiry email`
      );
    }
  }
